...
def run_deseq2(big_exp, small_exp, big_end, small_end, big_mid, small_mid, complete_exp, complete_end, complete_mid, man_input_list, transcript_to_gene, CRISPR, window, GNAR, srr_lab_exp, srr_input_list, srr_lab_end, srr_input_list_exp, srr_lab_mid, srr_list_mid):
	global output_genes, input_genes, saveas_
	if srr_input_list != "NA":
          end_labs = srr_input_list
          exp_labs = srr_input_list_exp
          print('end_tag_before')
          print(srr_lab_end)
          end_tag = [srr_lab_end] * len(end_labs)
          print('end_tag_after')
          print(end_tag)
          exp_tag = [srr_lab_exp]* len(exp_labs)
          mid_labs = srr_list_mid
          mid_tag = [srr_lab_mid] * len(mid_labs)
          ...
          if mid_labs !="NA":
           ...
           print(input)
           print(input_tag)
           input_int, input_tag=process_folders(inputs, input_tag, GNAR)
...
#	  most_common_platform='HiSeq2000'
	...
	output_int, output_tag=process_folders(outputs, output_tag, GNAR)
	print(output_int)
	print(output_tag)
	...
	if isinstance(input_genes, list):
	 input_genes = pd.DataFrame(input_genes, columns=["inputs"])
	...
	lbl_save.grid(row=4, column=8,pady=5)
	new_widg.append(lbl_save)
	saveas = customtkinter.CTkEntry(master=window, width=150)
	saveas.grid(row=5, column=8, pady=5, sticky="e", padx=5)
	...
	if CRISPR !='CRISPRa/i':
	  ...
	  btn_save.grid(row=6, column=8, padx=5, pady=5)
	  ...
	  btn_save.grid(row=7, column=8, padx=5, pady=5)
	  new_widg.append(btn_save)
	...
	btn_display_dataset1.grid(row=2, column=8, padx=5, pady=5)
	...
	btn_display_dataset2.grid(row=3, column=8, padx=5, pady=5)
	new_widg.append(btn_display_dataset2)
	return output_genes, input_genes, new_widg


def run_graph(big_exp, small_exp, big_end, small_end, big_mid, small_mid, complete_exp, complete_end, complete_mid, man_input_list, transcript_to_gene, genename, fitted, df1, processed_data, window, GNAR, srr_lab_exp, srr_input_list, srr_lab_end, srr_input_list_exp, srr_lab_mid, srr_list_mid):
...
#def run_umap(value, exp_labs, exp_tag, end_labs, end_tag, mid_labs, mid_tag, man_input_list, transcript_to_gene, fitted, df1, processed_data, window, GNAR):
def run_umap(big_exp, small_exp, big_end, small_end, big_mid, small_mid, complete_exp, complete_end, complete_mid, man_input_list, transcript_to_gene, fitted, df1, processed_data, window, GNAR, srr_lab_exp, srr_input_list, srr_lab_end, srr_input_list_exp, srr_lab_mid, srr_list_mid):
	global output_genes, input_genes, df2, processed_data1
